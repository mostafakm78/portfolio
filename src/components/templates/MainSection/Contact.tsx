...
export default function Contact({ contactRef }: ContactProps) {
  ...
  return (
    <section ref={contactRef} className="2xl:h-screen w-full lg:mt-0 my-12 flex flex-col items-center justify-around z-50">
      <div className=" h-10/12 md:w-10/12 w-11/12 flex lg:flex-row flex-col gap-10 items-center justify-center">
        <div className="w-full h-full flex items-center justify-center">
          <h2 className="xl:text-9xl lg:text-8xl md:text-7xl text-4xl text-secondary/80 drop-shadow-[10px_15px_5px_rgba(0,0,0,0.15)]">{t('contactme')}</h2>
        </div>
        <div className="w-full h-full gap-6 bg-background rounded-3xl shadow-2xl p-6 flex flex-col justify-around">
          <p className="xl:text-xl text-base text-secondary/80">{t('contactone')}</p>
          <p className="xl:text-xl text-base text-secondary/80">{t('contacttwo')}</p>
          <svg className="text-foreground/70 h-20 mx-auto rotate-18" width="98" height="129" viewBox="0 0 98 129" fill="none" xmlns="http://www.w3.org/2000/svg" role="presentation" aria-hidden="true">
            <path
          ...
          <ul className="flex lg:flex-wrap lg:flex-row flex-col items-center justify-center lg:gap-6 gap-2">
            <li className='w-full lg:w-auto'>
              <Link href="tel:+989169799533" target="_blank" className="flex items-center gap-2 justify-center xl:text-2xl p-4 bg-linear bg-linear-180 from-foreground/20 to-background shadow-lg rounded-xl group hover:from-foreground text-secondary duration-300 transition-colors cursor-pointer">
                <span>{t('phone')}</span>
                <IoCall />
              </Link>
            </li>
            <li className='w-full lg:w-auto'>
              <Link href="https://t.me/Mostafakamari78" target="_blank" className="flex items-center gap-2 xl:text-2xl justify-center p-4 bg-linear bg-linear-180 from-foreground/20 to-background shadow-lg rounded-xl group hover:from-foreground text-secondary duration-300 transition-colors cursor-pointer">
                <span>{t('telegram')}</span>
                <FaTelegram />
              </Link>
            </li>
            <li className='w-full lg:w-auto'>
              <Link href="https://wa.me/989169799533" target="_blank" className="flex items-center gap-2 xl:text-2xl justify-center p-4 bg-linear bg-linear-180 from-foreground/20 to-background shadow-lg rounded-xl group hover:from-foreground text-secondary duration-300 transition-colors cursor-pointer">
                <span>{t('whatsapp')}</span>
                <IoLogoWhatsapp />
              </Link>
            </li>
            <li className='w-full lg:w-auto'>
              <Link href="http://www.instagram.com/imyour_mosi" target="_blank" className="flex items-center gap-2 xl:text-2xl justify-center p-4 bg-linear bg-linear-180 from-foreground/20 to-background shadow-lg rounded-xl group hover:from-foreground text-secondary duration-300 transition-colors cursor-pointer">
            ...
            <li className='w-full lg:w-auto'>
              <Link href="https://github.com/mostafakm78" target="_blank" className="flex items-center gap-2 xl:text-2xl justify-center p-4 bg-linear bg-linear-180 from-foreground/20 to-background shadow-lg rounded-xl group hover:from-foreground text-secondary duration-300 transition-colors cursor-pointer">
                <span>{t('github')}</span>
                <FaGithub />
              </Link>
            </li>
      ...
      <p className="opacity-50 md:text-base text-sm text-center text-secondary 2xl:mt-0 mt-8">{t('rights')}</p>
    </section>
  );
}
