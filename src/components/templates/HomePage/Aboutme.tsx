...
export default function Aboutme({ aboutmeRef }: AboutmeProps) {
  ...
  return (
    <section ref={aboutmeRef} className={`lg:h-screen lg:mt-0 mt-12 w-full flex lg:flex-row flex-col z-50 ${locale === 'en' ? 'lg:flex-row-reverse' : ''} items-center justify-center gap-10 md:px-10 px-3`}>
      <div className="hidden w-4/12 h-10/12 lg:flex flex-col gap-8 items-center justify-center">
        <svg className="text-secondary/70 h-20 mx-auto rotate-45" width="98" height="129" viewBox="0 0 98 129" fill="none" xmlns="http://www.w3.org/2000/svg" role="presentation" aria-hidden="true">
          <path
        ...
        <Link href="/MostafaKamari-Resume.pdf" download className="flex items-center gap-2 text-xl text-secondary p-4 w-full justify-center bg-linear bg-linear-180 from-foreground/20 to-background shadow-lg rounded-xl group hover:from-foreground duration-300 transition-colors cursor-pointer">
          {t('download')}
        </Link>
      </div>
      <div className="pin lg:relative w-full flex flex-col h-10/12">
        <div className={`about h-[500px] relative lg:absolute z-30 flex flex-col items-center justify-around w-full md:h-full bg-background rounded-3xl shadow-2xl p-6 3xl:text-lg gap-2`}>
          <LiquidGlass className="aboutliquid p-4 md:text-2xl text-base rounded-4xl absolute -top-8 left-1/2 -translate-x-1/2 text-secondary">{t('aboutme')}</LiquidGlass>
          <p className="leading-7 md:p-6 p-2 text-secondary text-base md:overflow-y-hidden overflow-y-scroll">{t('aboutmefull')}</p>
        </div>
        <div className="skills h-[500px] lg:my-0 my-20 relative lg:absolute lg:translate-y-full lg:opacity-0 z-50 w-full md:h-full bg-background rounded-3xl shadow-[0px_-5px_10px_4px_rgba(0,0,0,0.1)] p-6 flex flex-col items-center justify-around gap-2">
          <div className="absolute -top-8 left-1/2 -translate-x-1/2">
            <LiquidGlass className="p-4 md:text-2xl text-base rounded-4xl text-secondary">{t('skills')}</LiquidGlass>
          </div>
          <div className="grid grid-cols-3 w-full gap-2 py-10">
            {apps.map((app: App) => {
              return (
                <div key={app.key} className={`flex flex-col justify-center items-center p-3 rounded-2xl border border-secondary gap-3`}>
                  <Image className="w-10 h-10" width={800} height={800} alt={app.name} src={app.src} />
                  <span className="text-[12px] lg:text-base text-secondary">{app.name}</span>
                </div>
              );
            })}
          </div>
